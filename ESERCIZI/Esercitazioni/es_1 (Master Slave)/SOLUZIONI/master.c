...
int main(int argc, char ** argv){
    ...
    for (int i = 1; i <= slaves; i++){
        strcpy(fifoMaster[i-1],PIPE_MASTER); //Copy base name
        ...
        printf("Creating pipes for slave\n");
        ...
        for (int j = i+1; j <= slaves; j++){
            ...
            //Delete file for fifo, and create the fifo
            unlink(tmp);
            mkfifo(tmp,0666);
        }
    ...
    //Execute the slaves
    char currentSlave[10],totSlaves[10];
    
    //Create the argument array containing the name of the executable
    // the number of current slave, the total number of slaves and the pipe root names
    char * argExec[] = {"./slave.out","0",totSlaves,PIPE_MASTER,PIPE_SLAVE,NULL};
    
    //Convert to string the number of slaves
    sprintf(totSlaves, "%d", slaves);
    
    printf("Creating slaves\n");
    for (int i = 1; i <= slaves; i++){
        //Pass as argument the number of the slave
        sprintf(currentSlave, "%d", i);
        argExec[1]=currentSlave;
        if(fork()==0) //Child
            execv(argExec[0],argExec); //Execute the slave!
    }
    ...
    while(1){
        ...
        for(int i = 0; i<r;i++){
            ...
            if(buffer[i]=='<'){
                //Save destination slave
                strncpy(tmp,buffer,i);
                tmp[i]=0; //terminate string
