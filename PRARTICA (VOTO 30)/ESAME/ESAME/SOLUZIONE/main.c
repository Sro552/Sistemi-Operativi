...
int createQueue(char *name, int force) {
    ...
    if (qnew<0) if (force>0) { tr=-1; } else { quit(7); };
...
int doMoveQueue(char *value) {
    ...
    if(qid2 <1) quit(6);
    while ((res=doReadQueue())>0) { // keep on reading messages from source
...
int perform(int chosen, char *value, int pid) {
    ...
    if (chosen==get) doReadQueue();         // read from current (source) queue
